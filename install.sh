...
source <(curl -sL https://raw.gitmirror.com/rustdesk/rustdesk-server-pro/main/lib.sh)
...
RDLATEST=$(curl https://api.kkgithub.com/repos/rustdesk/rustdesk-server-pro/releases/latest -s | grep "tag_name"| awk '{print substr($2, 2, length($2)-3) }')
...
Please report this to: https://kkgithub.com/rustdesk/rustdesk-server-pro/issues"
            exit 1
        fi
        ...
        # Download
        if ! curl -fSLO --retry 3 https://kkgithub.com/rustdesk/rustdesk-server-pro/releases/download/"${RDLATEST}"/rustdesk-server-linux-"${ACTUAL_TAR_NAME}".tar.gz
        then
